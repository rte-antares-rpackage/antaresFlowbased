...
By default, ``PTDF`` argument takes the value **BP2017**. In that case, the typical days used in the Bilan Previsionnel 2017 will be loaded in input.
...
To add flow-based time series file to this folder, the function ``addChroniquesFile()`` must be used. In this version of the package, the time series file can't be changed, they are the ones used in the Bilan Previsionnel 2017. The ``outputName`` specifies the directory where the time series files will be saved. We recommend to specify the same folder as the one given to ``computeFB()``.
...
Moreover, ``the antaresFlowbased`` package contains some already built domains, notably the ones used in the last BP. The data contained in the package can be seen with the function ``getAvailableBP()``. Thus, instead of loading an external directory with ``setFlowbasedPath()``, the internal data of the package can be used.
