...
* **weight.txt** : Coefficient of face for antares
* **second_member.txt** : Faces for each typical day and hour
...
To add chronics file to this folder, you must use function *addChroniquesFile()*. In this version of package, chronics file can't be change. The **outputName** specify the directory where chroniques files will be save. We recommend to specify same folder as *computeFB()* call.
...
You can also produce reports in html format with shiny application. This solution can be use to facilitate the sharing.
<div style="text-align:center" markdown="1">
...
**path** parameters can be use to specify a folder generate by *computeFB()* funciton.
...
This function uses  files contain in your optimisation forlder (create by *computeFB()* function) and specify with *setFlowbasedPath()* function.
