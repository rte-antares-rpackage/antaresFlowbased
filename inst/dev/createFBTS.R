...
probabilityMatrix = matProb


createFBTS(opts = opts, probabilityMatrix = matProb, multiplier = multiplier,
...
createFBTS <- function(opts, probabilityMatrix, multiplier,
  ...
  outTs$typicalDay <- sapply(1:nrow(outTs), function(R){
    ...
    quantilesClass <- quantiles[class == outTs1$class]
    
    #For all variables create mini request
    allReq <- sapply(sdC, function(Y){
      quantilesClassVar <- quantilesClass[, .SD, .SDcols = c("quantiles", Y)]
      ...
      if(nrow(quantilesClassVar) == 1){
        if(mynmber[[Y]] < quantilesClassVar[[Y]]){
      ...
      #If more row
      if(nrow(quantilesClassVar) > 1){
        if(mynmber[[Y]] < quantilesClassVar[[Y]][1]){
          littleReq <- paste0(Y, "== '0_", quantilesClassVar$quantiles[1], "'")
        } else if(mynmber[[Y]] > quantilesClassVar[[Y]][length(quantilesClassVar[[Y]])]){
          littleReq <- paste0(Y, "== '", quantilesClassVar$quantiles[nrow(quantilesClassVar)], "_1'")
        }else{
          firstSup <- which.min(mynmber[[Y]] > quantilesClassVar[[Y]])
    ...
    allReq
    allReq <- paste0(allReq, " & class == '", outTs1$class ,"'")
